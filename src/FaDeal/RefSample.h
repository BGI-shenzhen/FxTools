...
int  print_AusageFA02()
{
	cout <<""
		"\n"
		"\tUsage: extractN -i <in.fa>  -o <out.fa> -s <str>\n"
		"\n"
		"\t\t-i      <str>   input fa for geting  Sub range Se\n"
...
void More_HelpFM_EE()
{
	cout<<"\n\
\n\
\t\t1.  extractN -i <in.fa> -o AAA -s 1-3\n\
\t\tthis will extract the first to third sequences from FASTA and output the result to a compressed file named AAA in current directory.\n\
\n\
\t\t2.  extractN -i <in.fa> -o AAA -s 2,4\n\
\t\tthis will extract the second and the forth sequences from FASTA and output the result to a compressed file named AAA in current directory.\n\
\n\
\n";
}
...
{
	if (argc <=1 ) {print_AusageFA02();return 0;}
