...
int  usage_sort()
{
	cout <<""
		...
		"\t\t-a             ascending sort. default is [descending sort]\n"
...
\t\tthis will sort the input FASTA by length in descending way and output the result to a compressed file named AAA in current directory.\n\
\n\
\t\t3.  sort -i <in.fa> -o AAA -a\n\
\t\tthis will sort the input FASTA by name in descending way and output the result to a compressed file named AAA in current directory.\n\
\n\
\n";
}
...
int parse_Acmd_FA18(int argc, char **argv ,In3str1v * para_FA18 )
{
	...
	for(int i = 1; i < argc ; i++)
	{
		...
		else if (flag  == "reverse"  ||  flag  == "descend" ||  flag  == "d" ||   flag  == "a")
		{
			para_FA18->TF2=false;
		}
