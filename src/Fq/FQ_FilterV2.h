...
int  print_usage_A_B24 ()
{
	cout <<""
		"\n"
		"Usage:filter  -i <1.fq> <2.fq> -o <out.1> <out.2> \n"
		"\n"
		"\t\t-i      <str>     input PE FASTQ. For SE FASTQ, just omit <2.fq>\n"
		"\t\t-o      <str>     output for PE FASTQ. For SE FASTQ, just omit <out.2>\n"
		"\t\t-n      <float>   the max ratio of miss N for sequence to pass filter, default [0.1]\n"
		"\t\t-Q      <int>     standard of low quality score. Scores not greater than this value would be taken as low quality, default [5]\n"
		"\t\t-d      <float>    the ratio of low quality base (value of low quality is defined by -Q). Sequences with low quality bases more than this value would be removed, default [0.5]\n"
//		"\t\t-f                Cut off the index\n"
		"\t\t-s                trim the sequence by deleting the low quality bases in the beginning or end\n"
		"\t\t-l      <int>     standard of short sequence. Sequences shorter than this value after trimming would be removed, default [30]\n"
		"\n"
...
{
	if (argc <=1  ) {print_usage_A_B24();return 0;}
