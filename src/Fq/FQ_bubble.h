...
int  print_usage_C01 ()
{
	cout <<""
		"\n"
		"Usage:bubble -i  <1.fq> <2.fq> -o  <out.1> <out.2> -a <int> -b <int>\n"
		"\n"
		"\t\t-i    <str>   input PE FASTQ\n"
		"\t\t-o    <str>   output for PE FASTQ\n"
		"\t\t-a    <str>   the site(s) of N to filter the reads in <1.fq>\n"
		"\t\t-b    <str>   the site(s) of N to filter the reads in <2.fq>\n"
...
{
	cout<<"\n\
\t\t1.	bubble -i <1.fq> <2.fq> -o <out.1> <out.2> -a 3,8 -b 4\n\
\t\tthis will remove the reads which have N on site 3 or 8 from <1.fq> and the reads which have N on site 4 from <2.fq>, and output the results to two compressed files in current directory. So far this function only works for PE FASTQ.\n\
\n\
\n";
}
//
...
{
	if (argc <=1  ) {print_usage_C01();return 0;}
