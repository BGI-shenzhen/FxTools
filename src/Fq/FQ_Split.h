...
int  print_usage_Sp03 ()
{
	cout <<""
		...
		"\t\t-n    <int>   max number of sequences in each splitting output, default [10000000]\n"
...
void More_HelpFQ6()
{
	cout<<"\n\
\n\
\t\t1.  splitFq -i <in.fq> -o ./ -n X \n\
\t\tthis will split the input FASTQ and output the splitting files in current directory. Each output has at most X sequences, default value of X is 10000000.\n\
\n\
\n";
}
...
{
	if (argc <=1  ) {print_usage_Sp03();return 0;}
...
//int main(int argc, char **argv)
{
	...
	if (!(para->InFq2).empty())
	{
		...
		IN_1.close();
		OUT_1.close();
		cout<<"\t\tTotal Read PE Number: "<<read_Fisrt<<" ,and split to " <<Count<<" *2 files"<<endl;
	}
	...
	else
	{
		...
		cout<<"\t\tTotal Number Read "<<read_Fisrt<<",and split to " <<Count<<" files "<<endl;
