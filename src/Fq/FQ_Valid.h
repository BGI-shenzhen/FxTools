...
int  print_U_Valid()
{
	cout <<""
		...
		"\t\t-i    <str>     Input FASTQ\n"
		"\n"
		"\t\t-o    <str>     output a new valid FASTQ by removing the invalid sequences\n"
		"\t\t-w    <str>     show the IDs of invalid sequence\n" 
		"\n"
...
\t\tif the input FASTQ is invalid, this will remove the invalid sequences from the original FASTQ, keep the valid sequences left and output them to a file named AAA in current directory. If the user would like to see which sequences are removed, they could check this in the file BBB containing the IDs of invalid sequences.\n\
\n\
\n";
}
...
{
	if (argc <=1 ) {print_U_Valid();return 0 ;}
	...
	if (!(para_A30->InStr2).empty())
	{
		...
		while(!IN_A.eof())
		{
			...
			if (SeqLen!=s_Aend)
			{
				...
				Info<<"Sequence(s) with Error :"<<ID_1<<endl;
				OUTTF=false ;
			}
			if (s_Aend<1)
			{
...
//			Info<<"Sequence(s) with Error at Empty seq :"<<ID_2<<endl;
			Info<<"Sequence(s) with Error :"<<ID_2<<endl;
			OUTTF=false;
			OUTTFALLV2=false;
		}	
